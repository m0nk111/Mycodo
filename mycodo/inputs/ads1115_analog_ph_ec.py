...
# Measurements
measurements_dict = {
    0: {"measurement": "ion_concentration", "unit": "pH"},
    1: {"measurement": "electrical_conductivity", "unit": "uS_cm"},
}
...
INPUT_INFORMATION = {
    ...
    "custom_options": [
        ...
        },
        {
            ...
            "name": "{}: {}".format(
                lazy_gettext("Temperature Compensation"), lazy_gettext("Measurement")
            ),
        ...
        },
        {
            ...
            "constraints_pass": constraints_pass_positive_value,
            "name": "{}: {} ({})".format(
                lazy_gettext("Temperature Compensation"),
                lazy_gettext("Max Age"),
        ...
        {"type": "message", "default_value": "EC Calibration Data"},
        {
            "id": "ec_cal_v1",
            "type": "float",
            "default_value": 0.232,
...
class InputModule(AbstractInput):
    ...
    def calibrate_ph(self, cal_slot, args_dict):
        ...
        if "calibration_ph" not in args_dict:
            ...
            return
        if not isinstance(args_dict["calibration_ph"], float) and not isinstance(
            args_dict["calibration_ph"], int
        ):
            self.logger.error(
                "buffer value does not represent a number: '{}', type: {}".format(
    ...
    def calibrate_ec(self, cal_slot, args_dict):
        ...
        if "calibration_ec" not in args_dict:
            ...
            return
        if not isinstance(args_dict["calibration_ec"], float) and not isinstance(
            args_dict["calibration_ec"], int
        ):
            self.logger.error(
                "standard value does not represent a number: '{}', type: {}".format(
        ...
        if args_dict["calibration_ec"] > 0:
            deviation_pct = (
                abs(verify_ec - args_dict["calibration_ec"])
                / args_dict["calibration_ec"]
                * 100
            )
        else:
            deviation_pct = 0
    ...
    def get_temp_data(self):
        ...
        if self.temperature_comp_meas_measurement_id:
            ...
            if last_measurement and len(last_measurement) > 1:
                ...
                _, unit, _ = return_measurement_info(device_measurement, conversion)
                ...
                self.logger.debug("Latest temperature: {temp} C".format(temp=out_value))
            else:
                self.logger.error(
    ...
    def convert_volt_to_ph(self, volt, temp):
        ...
        self.intercept = self.ph_cal_ph1 - self.slope * self.nernst_correction(
            self.ph_cal_v1, self.ph_cal_t1
        )
        if temp is not None:
    ...
    def convert_volt_to_ec(self, volt, temp):
        ...
        self.intercept = self.ec_cal_ec1 - self.slope * self.viscosity_correction(
            self.ec_cal_v1, self.ec_cal_t1
        )
        if temp is not None:
            # Perform temperature corrections
